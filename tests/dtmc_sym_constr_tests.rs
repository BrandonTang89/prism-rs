...
fn construct_symbolic_dtmc(model_str: &str) -> Result<SymbolicDTMC> {
    let mut ast = parse_dtmc(model_str)?;
    let info = prism_rs::analyze::analyze_dtmc(&mut ast)?;
...
fn test_construction(
    ...
    let mut symbolic_dtmc =
        construct_symbolic_dtmc(&model_str).expect("Failed to construct symbolic DTMC");
...
fn dtmc_simple_constr() {
    // regression test
    test_construction("tests/dtmc/simple_dtmc.prism", 9, 2, 6, 3);
}

#[test]
fn dtmc_knuth_die_constr() {
    // regression + prism comparison test
    test_construction("tests/dtmc/knuth_die.prism", 65, 3, 20, 13);
}

#[test]
fn dtmc_knuth_two_dice_constr() {
    // regression + prism comparison test
    test_construction("tests/dtmc/knuth_two_dice.prism", 274, 3, 79, 45);
}
