...
impl RefManager {
    ...
    fn track_ref(&mut self, node: NodeId) {
        let reg = NodeId(unsafe { Cudd_Regular(node.as_ptr()) });
    ...
    pub fn read_var_index(&self, node: NodeId) -> u16 {
        let reg = NodeId(unsafe { Cudd_Regular(node.as_ptr()) });
        if unsafe { Cudd_IsConstant(reg.as_ptr()) } != 0 {
            u16::MAX
        } else {
    ...
    pub fn read_else(&self, node: NodeId) -> NodeId {
        ...
        if unsafe { Cudd_IsConstant(reg.as_ptr()) } != 0 {
            ...
            if unsafe { Cudd_IsComplement(node.as_ptr()) } != 0 {
    ...
    /// `Cudd_addIte` requires ADD condition.
    /// __Refs__: result; __Derefs__: cond,then,else
    pub fn add_ite(
        &mut self,
        cond: BddNode,
        then_branch: AddNode,
    ...
    pub fn dag_size(&self, root: NodeId) -> usize {
        let root = NodeId(unsafe { Cudd_Regular(root.as_ptr()) });
    ...
    pub fn terminal_nodes(&self, root: NodeId) -> Vec<NodeId> {
        ...
        self.foreach_node(root, |n| {
            if unsafe { Cudd_IsNonConstant(n.as_ptr()) } == 0 {
                out.push(NodeId(unsafe { Cudd_Regular(n.as_ptr()) }));
            }
        });
    ...
    pub fn dump_add_dot(
        ...
        let root_reg = NodeId(unsafe { Cudd_Regular(root.0.as_ptr()) });
    ...
    fn dump_add_dot_rec<W: Write>(
        ...
        let n = NodeId(unsafe { Cudd_Regular(n.as_ptr()) });
        if !visited.insert(n) {
            return Ok(());
        }
        ...
        let t = NodeId(unsafe { Cudd_Regular(self.read_then(n).as_ptr()) });
        let e = NodeId(unsafe { Cudd_Regular(self.read_else(n).as_ptr()) });
